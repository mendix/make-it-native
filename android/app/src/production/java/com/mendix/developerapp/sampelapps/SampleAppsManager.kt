...
class SampleAppViewInfo(val id: String, val name: String, val description: String, val splashFilePath: String, val runtimeUrl: String)
...
class SampleAppsManager(context: Context, documentsPath: String) {
    ...
    var sampleAppsProvider: SampleAppsProvider
        private set
    val appsCount get() = this.sampleAppsProvider.appsCount
    ...
    fun getSampleAppInfo(index: Int): SampleAppViewInfo {
        sampleAppsProvider.getApp(index).let {
            return SampleAppViewInfo(it.id, it.name, it.description, resolvePreviewImagePath(it.id), it.runtimeUrl)
        }
    }
