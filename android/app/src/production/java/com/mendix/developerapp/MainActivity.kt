...
class MainActivity : AppCompatActivity(), DefaultHardwareBackBtnHandler, LaunchScreenHandler,
    ...
    private val appUrlObserver = Observer<String> {
        preferences.appUrl = it
        MxConfiguration.runtimeUrl = AppUrl.forRuntime(it)
    }
    ...
    override fun onResume() {
        super.onResume()
        homeViewModel.appUrl.observe(this, appUrlObserver)
    ...
    override fun onPause() {
        ...
        homeViewModel.appUrl.removeObserver(appUrlObserver)
        super.onPause()
    }
    ...
    private fun setupAppViewModel() {
        ...
        homeViewModel.arUnsupportedMessageShown.observe(this) { shown ->
            devAppPreferences.edit().putBoolean(PREF_AR_ERROR_SHOWN, shown).apply()
        }
    }
        ...
        navHostFragment.navController.navigate(R.id.nav_mendix_app, Bundle().also {
            ...
            it.putSerializable(
                MendixReactFragment.ARG_MENDIX_APP, MendixApp(
                    homeViewModel.appUrl.value!!,
                    getWarningFilterValue(this),
                    homeViewModel.devModeEnabled.value ?: false,
                    true
                )
            )
