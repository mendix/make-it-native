...
5. Zip the app folders by selecting all of them and then zip all app folders; rather than zipping the parent directory. Zipped folder should be called `sampleapps.zip`. Note: make sure that the root content of the `.zip` is each sample app folder
5. In "A", open `sample_apps.json` and bump the version
